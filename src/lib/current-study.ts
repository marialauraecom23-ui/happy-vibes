...
export type StudySession = {
  ...
  week: number;
  weekday: number;
  state: "before-start" | "upcoming-today" | "in-progress" | "next-day" | "after-exam";
};

/** Resolves which plan day the app should present, based on the device clock. */
export function currentSession(now = new Date()): StudySession {
  const start = new Date(`${PLAN_START}T00:00:00`);
  const key = localDateKey(now);
  let date = new Date(`${key}T00:00:00`);
  let state: StudySession["state"] = "in-progress";
  ...
  if (key < PLAN_START) {
    ...
    state = "after-exam";
    date = new Date(`${EXAM_DATE}T00:00:00`);
  } else if (now.getHours() < STUDY_START_HOUR) {
    state = "upcoming-today";
  } else if (now.getHours() >= DAILY_END_HOUR) {
    const next = new Date(date);
    next.setDate(next.getDate() + 1);
    if (localDateKey(next) <= EXAM_DATE) {
      date = next;
      state = "next-day";
    } else {
      state = "after-exam";
...
export function shortPlanDate(date: Date) {
  return new Intl.DateTimeFormat("pt-BR", { day: "2-digit", month: "2-digit" }).format(date);
}

export function planWeekDates(session: StudySession) {
  const first = new Date(session.date);
  first.setDate(first.getDate() - session.weekday);
